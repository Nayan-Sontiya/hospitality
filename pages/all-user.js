...
function AllUser() {
  ...
  useEffect(() => {
    const { occupationName, salaryEnd } = router.query;
    setOccupationName(occupationName);
    setSalEnd(salaryEnd);
    setSalStart(salaryStart);
  }, [router.isReady]);

  useEffect(() => {
    GetSearchData(location, true);
  }, [Location, occupationName, category, salStart, salEnd, location]);
  ...
  // useEffect(() => {
  //   window && window.addEventListener("scroll", infiniteScroll);
  //   // infiniteScroll();
  // }, []);
  ...
  const GetSearchData = async (locationFirst, newData) => {
    // setPage((pre) => pre + 1);
    // if (occupation) {
    ...
    //     `getFilteredCandidatesByOccupation?page=${page + 1}&perPage=${10}`,
    //     item
    //   );
    ...
    let item = {
      ...
      sub_category: sub_category.length > 0 ? sub_category : undefined,
    };
    let p = newData ? -1 : page;
    if (newData) {
      setPage(0);
    }
    let response = await PostRequest(
      `getFilteredCandidates?page=${p + 1}&perPage=${10}`,
      item
    );
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title>
          {profiles.find((profile) => profile.category === occupationName)
            ?.title ?? `Hospitality Finder`}
        </title>
        <meta
          name="description"
          content={
      ...
      {process.browser ? (
        <>
          ...
          <div className="grid grid-cols-12 gap-y-5">
            <div
              className={`col-span-12  md:col-span-3 md:block md:border-r border-[#B7B7B7] md:border-y-0 md:border-l-0 border rounded-md px-2 mx-3 md:m-0 md:rounded-none`}
            >
              ...
              <div
                className={`${Style.userDetailsContainerHeight} px-3 ${
                  isFilterHidden ? "hidden" : "block"
                } block md:hidden`}
              >
